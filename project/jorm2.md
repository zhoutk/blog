...
## 背景
我要完成以json为数据媒介，来操作数据和网络数据传输。查资料，rapidjson是比较流行的json库，并且速度快。但在以我的使用方式，用起来非常麻烦，而且我的目的是数据交换。rapidjson非常普通的，看起来应该是值传输的操作，其实都是内存移动。这虽然能达到高效率，但一不小心就会出错，而且写出来非常的丑陋，所以我写了个代理类，来达到我的需求。
...
## 感受
这是封装数据库通用访问接口的第一步，提供方便的json对象操作，不然写个json对象得郁闷死。很怀念在javascript中操作json的感觉，如丝润滑......
